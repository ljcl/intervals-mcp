 ...
 * the most recent response from the retired, transitional Strava client
 * (`fetchClient.ts`'s `stravaApi`, still exercised by the handful of tools
 * not yet ported to intervals.icu), not intervals.icu itself, so the
 * endpoint never spends an intervals.icu request.
 *
