 ...
 * That is accurate and useless: a model copying an id that has grown past
 * 2^53 sees a number-shaped id and emits the number branch, the host's
 * `JSON.parse` rounds it on the way in, and the call fails validation with
 * the true digits already unrecoverable. Neither the branch's `maximum` nor
 * the description reliably steers generation away from the number form.
 *
 ...
 *
 * Covers every `intervalsActivityIdInput` schema, narrowed to its pattern
 * via `idSchemas`.
 *
 * Pass to `z.toJSONSchema(..., { override })`.
 */
export function idJsonSchemaOverride(ctx: {
 ...
 * generation toward the quoted string form the schema advertises.
 *
 ...
 * went through `intervalsActivityIdInput`.
 */
 ...
 * normalised to a string. See `intervalsActivityIdInput` for the full
 * rationale.
 */
 ...
 * also accepted. An id sent as a JSON number can lose precision in the
 * host's JSON round-trip before validation ever sees it if it has grown past
 * `Number.MAX_SAFE_INTEGER` (2^53 - 1), so the digit-string form is the only
 * lossless representation, and the only one advertised to hosts
 ...
 *   reaches zod it has already been rounded by the host's `JSON.parse`, so
 *   accepting it would silently fetch the wrong resource or, far more
 *   likely, 404. The error names the rounded value and asks for the original
 *   digits as a string, because that is the only thing the caller can act on.
 *
