 ...
 * that 401 comes from `stravaClient.ts` sending no Authorization header on
 * purpose, not from intervals.icu rejecting a real key.
 *
 * Imports come from `../fetchClient` only (including `NotPortedError`, which
 * lives there rather than in `stravaClient.ts` for this reason). Tool tests
 * replace `../stravaClient` with bare factory mocks, so anything imported
...
export function toolErrorText(
  ...
  if (error instanceof NotPortedError) {
    ...
    // HttpError with status 401 (stravaClient.ts sends no Authorization
    // header on purpose), and that 401 means "not yet ported to
